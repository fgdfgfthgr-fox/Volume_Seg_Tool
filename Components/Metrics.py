...
class BinaryMetrics(nn.Module):
    ...
    def forward(self, inputs, targets, smooth=1e-8):
        ...
        tpr = true_positives / (true_positives + false_negatives + smooth)
        tnr = true_negatives / (true_negatives + false_positives + smooth)

        if dice_score >= 0.995 or dice_score <= 0.001:
            print('Dice Score Issue! Check Aug!')
