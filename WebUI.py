        ...
        with gr.Tab("Main"):
            ...
            with gr.Row():
                ...
                with gr.Tab("Predict Settings"):
                    ...
                    with gr.Row():
                        predict_hw_size = gr.Number(128, label="Height and Weight of each Patch (px)", precision=0, step=16)
                        predict_depth_size = gr.Number(128, label="Depth of each Patch (px)", precision=0, step=16)
                        predict_hw_overlap = gr.Number(8, label="Expansion in Height and Weight for each Patch (px)", precision=0, step=8)
